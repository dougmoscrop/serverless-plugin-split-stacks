...
module.exports = {
  ...
  migrate(logicalId, destination, allowSuffix) {
    const stackName = this.getStackName(destination, allowSuffix);

    if (logicalId in this.resourceMigrations) {
      const migration = this.resourceMigrations[logicalId];

      if (migration.stackName.indexOf(destination) === 0) {
        return;
      }

      throw new Error('${logicalId} was already migrated to ${migration.stackName}');
    }
  ...
  getStackName(destination, allowSuffix) {
    ...
    while (stackName === undefined) {
      const potentialStackName = `${destination}NestedStack${stackSuffix}`;
      ...
      if (Object.keys(stack.Resources).length < 200) {
        ...
        if (allowSuffix) {
          if (stackSuffix) {
            stackSuffix = Number(stackSuffix) + 1
          } else {
            stackSuffix = 2;
          }
        } else {
  ...
  getNestedStackFiles() {
    ...
    return fs.readdirSync(serverlessDirPath)
      .filter(filename => filename.match(/nested-stack\.json$/))
      .map(filename => {
        return {
          name: filename,
